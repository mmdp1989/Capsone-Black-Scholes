...
bs_present = bs('p', s_present, k, t, r, sigma)
stocks_with_hedge = 1000 * v1
options_with_hedge = 1000 * (1 - v1)
hedge_stocks_cost = s_present * stocks_with_hedge
options_cost = bs_present * options_with_hedge

c = []
for i in range(len(s_future)):
    c.append(((s_future[i] * stocks_with_hedge) - hedge_stocks_cost) + ((bs('p', s_future[i], k, t, r, sigma) * options_with_hedge) - options_cost))
...
plt.suptitle("Delta hedging 1000 stocks using puts") 
